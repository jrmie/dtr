...
# 1 #
display_numeric <- function(x, group, grp_levels, var.num, stat.num, spec.var, digits){
  
  # For loop to apply rounding
  for (i in var.num) {
    if(i %in% names(spec.var) && !is.null(spec.var[[i]]$digit)){
      n_digits <- as.numeric(spec.var[[i]]$digit)
    } else {
      n_digits <- digits
    }
...
# 2 #
display_factor <- function(x, grp_levels, var.fct, spec.var){
...
# 4 #
display_order <- function(data, grp_levels, var.fct, spec.var, compare){
  ...
  #columns
   ...
   #rows
   if(!is.null(spec.var)){
     order_rows <- c(names(spec.var), names(data)) %>% unique()
     } else {
...
# 5 #
display_names <- function(var.num, var.fct, spec.var){
  var.names <- map_dfr(
    c(var.num, var.fct),
    ~ list(variable = .x, name = ifelse(is.null(spec.var[[.x]]$name),
                                        .x,
                                        spec.var[[.x]]$name)
           )
    )
